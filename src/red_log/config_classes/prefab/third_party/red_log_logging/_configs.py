"""The classes and functions in this file define logging for the `red_log` library.

If you want to see messages from `red_log` modules in your app, you will need to add
...
log = logging.getLogger("red_log.config_classes.prefab.third_party.red_log_logging")

import typing as t

from red_log.config_classes.formatters import FormatterConfig
from red_log.config_classes.handlers import (
    FileHandlerConfig,
    QueueHandlerConfig,
    QueueListenerConfig,
...
from red_log.config_classes.loggers import (
    LoggerConfig,
    LoggerFactory,
)
from red_log.fmts import (
    DATE_FMT_DATE_ONLY,
    DATE_FMT_STANDARD,
    DATE_FMT_TIME_ONLY,
)
from red_log.fmts._formats import (
    RED_LOG_DETAIL_FMT,
    RED_LOG_FMT,
)
from red_log.helpers import (
...
def get_red_log_formatter(
    name: str = "red_log", fmt: str = RED_LOG_FMT, datefmt: str = DATE_FMT_STANDARD
) -> FormatterConfig:
    """Return a pre-configured FormatterConfig for the red_log library.
    ...
    try:
        ...
        msg = Exception(
            f"Unhandled exception getting red_log FormatterConfig. Details: {exc}"
        )
        log.error(msg)
...
def get_red_log_console_handler(
    name: str = "red_log_console",
    level: str = "DEBUG",
    formatter: str = "red_log",
    filters: list[str] | None = None,
) -> StreamHandlerConfig:
    """Return an initialized StreamHandlerConfig for the red_log library.
    ...
    try:
        ...
        msg = Exception(
            f"Unhandled exception getting red_log StreamHandlerConfig. Details: {exc}"
        )
        log.error(msg)
...
def get_red_log_logger(
    name: str = "red_log",
    handlers: list[str] = ["red_log_console"],
    level: str = "WARNING",
    propagate: bool = False,
) -> LoggerConfig:
    """Return an initialized LoggerConfig for the red_log library.
    ...
    try:
        ...
        msg = Exception(
            f"Unhandled exception getting red_log LoggerConfig. Details: {exc}"
        )
        log.error(msg)
