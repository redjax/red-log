...
RED_LOG_FMT: str = str(
    "[%(asctime)s] [%(levelname)s] [(red_log).%(module)s:%(lineno)d > %(funcName)s()]: %(message)s"
)
RED_LOG_DETAIL_FMT: str = str(
    "[%(asctime)s] [%(levelname)s] [(red_log).%(module)s] [path:%(pathname)s:%(lineno)d] [method:%(funcName)s()]: %(message)s"
)
