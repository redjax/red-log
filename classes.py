...
class LoggingHandler:
    ...
    def get_handler_dict(self) -> dict:
        ...
        if self.stream and self.filename:
            if self.class_name == "logging.StreamHandler":
                _dict[self.name]["stream"] = self.stream
            elif self.class_name == "logging.FileHandler":
                _dict[self.name]["filename"] = self.filename
...
class LoggingConfig:
    ...
    def get_logger(self, name: str = None) -> logging.Logger:
        assert name, ValueError(
            "Missing a logger name. Pass name=__init__ to use module's name."
        )
